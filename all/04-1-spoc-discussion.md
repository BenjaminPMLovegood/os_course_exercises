...
及时delete或者GC，或者干脆存到文件系统里面。

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？

划分程序逻辑块，向不会同时用到的部分分配同一块内存。需要手动划分模块和确定模块之间的关系。

2. 什么是交换技术？覆盖与交换有什么不同？

交换是将内存中整个进程换入和换出。
覆盖是将程序分块，单位是逻辑块。而交换是针对进程。
...
时间局部性：被访问过的代码/数据在一段时间内可能被再次访问。  
空间局部性：被访问过的代码/数据的相邻代码/数据更可能被访问。  
分支局部性：一个分支语句这次很可能选择和上次一样的分支。
